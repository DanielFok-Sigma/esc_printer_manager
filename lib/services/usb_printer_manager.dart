...
class USBPrinterManager extends PrinterManager {
    ...
    if (Platform.isWindows) {
      try {
        ...
        if (OpenPrinter(szPrinterName, phPrinter, nullptr) == FALSE) {
          EscPrinterManager.logger.error("can not open");
          isConnected = false;
          printer.connected = false;
          return Future<ConnectionResponse>.value(
              ConnectionResponse.printerNotConnected);
        } else {
          EscPrinterManager.logger.info("szPrinterName: $szPrinterName");
      ...
      if (usbDevice != null) {
        debugPrint("vendorId $vendorId, productId $productId ");
        isConnected = true;
        printer.connected = true;
        return Future<ConnectionResponse>.value(ConnectionResponse.success);
      } else {
  ...
  Future<ConnectionResponse> disconnect({Duration? timeout}) async {
    if (Platform.isWindows) {
      ...
      EscPrinterManager.logger.error("disconnect");
  ...
  Future<ConnectionResponse> writeBytes(List<int> data,
    ...
    if (Platform.isWindows) {
      try {
        if (!isConnected) {
          await connect();
          EscPrinterManager.logger.info("connect()");
        }

        // Inform the spooler the document is beginning.
        final dwJob = StartDocPrinter(hPrinter, 1, docInfo!);
        if (dwJob == 0) {
          EscPrinterManager.logger.error("dwJob == 0");
          ClosePrinter(hPrinter);
          return ConnectionResponse.printInProgress;
        }
        // Start a page.
        if (StartPagePrinter(hPrinter) == 0) {
          EscPrinterManager.logger.error("StartPagePrinter == 0");
        ...
        if (WritePrinter(hPrinter, lpData, dwCount!, dwBytesWritten!) == 0) {
          EscPrinterManager.logger.error("WritePrinter == 0");
        ...
        if (EndPagePrinter(hPrinter) == 0) {
          EscPrinterManager.logger.error("EndPagePrinter == 0");
          EndDocPrinter(hPrinter);
          ClosePrinter(hPrinter);
        }
        ...
        if (EndDocPrinter(hPrinter) == 0) {
          EscPrinterManager.logger.error("EndDocPrinter == 0");
          ClosePrinter(hPrinter);
        }

        // Check to see if correct number of bytes were written.
        if (dwBytesWritten!.value != dwCount) {
          EscPrinterManager.logger.error("dwBytesWritten.value != dwCount");
        }
      ...
      EscPrinterManager.logger("start write");
      ...
      EscPrinterManager.logger("end write bytes.length${bytes.length}");
